...
Setting up GoogleTest was supposed to be my next goal before I got sidetracked by this Zig migration. An obvious convention would be to give the main package and each module its own set of tests (and therefore its own test executable), so that's what I did.
