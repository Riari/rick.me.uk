...
**build.zig** is a Zig source file contains a `build` function that accepts a pointer to the current build graph, allowing various operations (such as defining new modules and executables) to be carried out on the graph at build time.
