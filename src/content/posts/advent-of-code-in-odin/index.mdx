...
First up were the <ExternalLink url="https://code.visualstudio.com/docs/editor/tasks" label="tasks" />. I wanted to be able to do three things with these:
...
```
<div class="caption">`.vscode/tasks.json`</div>

(Note the "inputs" section—this is a useful feature that allows you to accept input from the user via a prompt.)

Next, the <ExternalLink url="https://code.visualstudio.com/docs/editor/debugging#_launch-configurations" label="launch configurations" />. I just needed two—one for debugging a solution, and another for debugging its tests.
...
```
<div class="caption">`.vscode/launch.json`</div>

<Alert type="tip">
The `lldb` configuration type is provided by the <ExternalLink url="https://marketplace.visualstudio.com/items?itemName=vadimcn.vscode-lldb" label="CodeLLDB" /> extension and the `cppvsdbg` type is provided by the <ExternalLink url="https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools" label="C/C++" /> extension.
</Alert>
