...
It's been a while since I [last posted back in May](/posts/2025/05/devlog-flecs-city-part-2-library-changes/). A lot has happened since then: Hytale was cancelled, I was back in the job market for the first time in eight years, I spent a few months focusing on sharpening my skills and building my portfolio, and I was lucky enough to catch the interest of several studios. Ultimately I accepted an offer from one of those studios and I'm now working full time on an Unreal Engine project.
...
I wrote two implementations of this--one for the server and one for the client. They implement `Init()` and `HandleEvent(const ENetEvent& event)`, and the client additionally implements `Connect(const char* address, uint32_t port)` and `Disconnect()` methods for (dis)connecting during runtime.

A quick test reveals successful connections in the server log:
```
...
There's a lot more to do here still, not least of which is sending/receiving data, which is probably what I'll focus on next. Once I have a reasonable foundation there, I'll think about how to handle replication of ECS components.
